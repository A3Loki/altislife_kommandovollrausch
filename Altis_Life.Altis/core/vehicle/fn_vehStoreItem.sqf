...
if(_num < 1) exitWith {hint "Unter 1 kannst du nichts lagern!";};
...
if(_ctrl == "money") then
{
	...
	if(CASH < _num) exitWith {hint "So viel Geld hast du nicht!"};
	if(_index == -1) then
	{
...
{
	if(((_totalWeight select 1) + _itemWeight) > (_totalWeight select 0)) exitWith {hint "Das Fahrzeug oder dein Inventar ist voll."};

	if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint "Items konnten nicht entfernt werden.";};
	_index = [_ctrl,_inv] call TON_fnc_index;
	if(_index == -1) then
	{
		_inv pushBack [_ctrl,_num];
