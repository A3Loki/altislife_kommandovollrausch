...
if(life_cgar_inUse+(300) >= time) exitWith {closeDialog 0;hint format["Du kannst nur alle 5 Minuten ein Fahrzeug verkaufen, nächstes in %1:%2",4 - floor ((time - life_cgar_inUse) / 60),59 - round (time - life_cgar_inUse - (floor ((time - life_cgar_inUse) / 60)) * 60)];};
